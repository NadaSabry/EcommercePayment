...
class FawryPay {
	...
	static captureOrderInfo(chargeRequest, config, accessToken) {
		...
		// make post request for init payment wit 
	...
	static loadPlugin(paymentId, config) {
		...
		//'https://interactive-card-details-ecru.vercel.app/';
		//`${fawry_plugin_origin + fawryplugin_deploy_path}?payment-id=${paymentId}&locale=${config.locale || 'en'}&mode=${config.mode || DISPLAY_MODE.SEPARATED}`;
